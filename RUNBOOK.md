...
1. **`.agent/` and `.agent/tools/` duplicates.** Two module pairs (`graduate`, `recall`) live in both locations with diverged implementations (the `auto_dream` duplicate was deleted 2026-05-18 — see item 1a). `.agent/tools/recall.py` is dead — `orchestrator.py` line ~64 explicitly says so, and `learning.py` replaced it. The other duplicates have callers on both sides, including user CLI invocations documented in `.agent/README.md`. Picking a single canonical path requires manually running each CLI to verify behaviour.

1a. **Learning-loop producer cleanup (resolved 2026-05-18).** The legacy `.agent/auto_dream.py`, `.agent/memory/auto_dream.py`, and `.agent/cluster_patterns.py` were deleted. They depended on a `.agent/memory/episodic/episodes.jsonl` no producer populated, so they never worked. The active producer is `gme_trading_system/lesson_producer.py` (scheduled at 16:35 ET daily in `orchestrator.py`). It reads SQLite (`signal_scores`, `predictions`, etc.) directly.
...
8. **`log_signal` hook in `episodic_integration.py` is still orphaned** as of 2026-05-18. `log_futurist_prediction` was wired 2026-05-18 into `orchestrator.run_futurist_prediction_signal` after `write_log` succeeds (soft-fail try/except, mirrors the Synthesis pattern). `log_synthesis` was wired earlier. `log_signal` remains deferred — its schema mismatches what `notify_signal_alert` provides, and `lesson_producer.py` reads `signal_scores` from SQLite directly so it's not blocking the loop. `log_manager_trade` is a deeper rabbit-hole — system has no autonomous execution, so the hook only fires if the human-in-the-loop trade flow ever logs back.
